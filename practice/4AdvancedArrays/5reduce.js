...
function vowelCount(str){
    ...
    return arr.reduce(function(accumulator,nextValue){
        if(vowels.indexOf(nextValue) !== -1){
            if(accumulator[nextValue]){ // if nextValue in Accumulator
...
function addKeyAndValue(arr, key, value){
    return arr.reduce(function(accumulator,nextValue,index){
        // accumulator[0]['title'] = 'Instructor'
...
value is true, the value should be placed in the first subarray. If the 
result of the callback function at that specific value is false, the 
